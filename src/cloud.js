...
import { computeShares } from './money.js?v=7';
  ...
  async snapshot() {
    ...
    (snap.groups || []).forEach((g) => {
      ...
      (g.members || []).forEach((m) => {
        ...
        if (!seen.has(m.id)) {
          seen.add(m.id);
          members.push({ id: m.id, name: m.display_name, initials: initialsOf(m.display_name), you: !!m.is_me, status: m.status });
        }
      });
      groups.push({ id: g.id, name: g.name, type: g.type, description: g.description, memberIds, archived: false });
    ...
    return { currentUserId: null, members, groups, expenses, proposedPayments, confirmedPayments, activity: [] };
  }
